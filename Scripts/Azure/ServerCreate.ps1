...
$publicIpAddress = New-AzPublicIpAddress -ResourceGroupName $resourceGroupName -Name "DC01" -AllocationMethod Dynamic -Location $location

# Create a network security group
$nsg = New-AzNetworkSecurityGroup -ResourceGroupName $resourceGroupName -Name "MyNSG" -Location $location

# Allow RDP traffic
$nsg | Add-AzNetworkSecurityRuleConfig -Name "Allow-RDP" -Priority 100 -Direction Inbound -Access Allow -Protocol Tcp -Priority 100 -SourceAddressPrefix Internet -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 3389
$nsg | Set-AzNetworkSecurityGroup

# Create a virtual network interface
$nic = New-AzNetworkInterface -ResourceGroupName $resourceGroupName -Name "MyNIC" -Location $location -SubnetId $virtualNetwork.Subnets[0].Id -PublicIpAddressId $publicIpAddress.Id -NetworkSecurityGroupId $nsg.Id
